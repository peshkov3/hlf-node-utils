...
import { ChainBaseService } from './chain.base.service';
import hlf = require('fabric-client');
import { FabricOptions } from '../models/fabricoptions.model';

export class ChainService extends ChainBaseService {
    ...
    invoke(fcnRequest: string, argsRequest: string[], ccId: string): Promise<any> {
        return Promise.resolve().then(() => {
            ...
            if (this.isProposalGood(results)) {
                ...
                Utils.logger.error('Failed to send Proposal or receive valid response. Response null or status is not 200. exiting...');
                throw new Error('Failed to send Proposal or receive valid response. Response null or status is not 200. exiting...');
            }
        }, (err) => {
            Utils.logger.error('Failed to send proposal due to error: ' + err.stack ? err.stack : err);
            throw new Error('Failed to send proposal due to error: ' + err.stack ? err.stack : err);
        }).then((response) => {
            if (response.status === 'SUCCESS') {
                Utils.logger.info('Successfully sent transaction to the orderer.');
                return this.txId.getTransactionID();
            } else {
                Utils.logger.error(`Failed to order the transaction. Error code: ${response.status}`);
                throw new Error(`Failed to order the transaction. Error code: ${response.status}`);
            }
        }, (err) => {
            Utils.logger.error('Failed to send transaction due to error: ' + err.stack ? err.stack : err);
